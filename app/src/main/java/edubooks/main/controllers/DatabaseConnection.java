...
//      THIS CHECKS IF ANY RECORDS WERE CHANGED RETURN TRUE IF IT DID FALSE IF NOT
    ...
    public Cursor listingBookSearchQuery(String sQuery){
        SQLiteDatabase DB = this.getReadableDatabase();
        return DB.rawQuery("SELECT Title,Author,IsbnNumber,Price,id FROM ListedBook WHERE (Title LIKE '%' || ? || '%' OR Faculty LIKE '%' || ? || '%' OR IsbnNumber = ?) AND isAvailible = 1",new String[]{sQuery,sQuery,sQuery});
    }
}